...
def test_translation_service_caches_successful_results_until_cleared(monkeypatch) -> None:
    service = AzureTranslationService(api_key="test-key", region="westeurope")
...
def test_translation_service_reuses_cached_results_in_batches(monkeypatch) -> None:
    service = AzureTranslationService(api_key="test-key", region="westeurope")
    fake_client = _FakeClient(_FakeResponse())
    monkeypatch.setattr(service, "_ensure_client", lambda: fake_client)
