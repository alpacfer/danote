...
def _init_nlp(
    ...
    app.state.nlp_enabled = settings.nlp_enabled
    ...
    if settings.nlp_enabled:
        try:
            ...
            logger.exception(
                ...
                extra={"nlp_model": settings.nlp_model},
            )
    else:
        app.state.nlp_ready = False
...
def _init_cor_local(app: FastAPI, settings: Settings) -> None:
    app.state.cor_local_lexicon_service = CORLocalLexiconService(
        db_path=settings.cor_local_db_path
    )
    ...
    if settings.cor_lookup_enabled:
        try:
            app.state.cor_lexicon_service = CORLexiconService(
                timeout_seconds=settings.cor_lookup_timeout_seconds
            )
        except Exception:
...
def _init_typo(app: FastAPI, settings: Settings) -> None:
    typo_engine = None
    if settings.typo_enabled and app.state.db_ready:
...
def _log_startup(app: FastAPI, settings: Settings, applied: list[str]) -> None:
    adapter = app.state.nlp_adapter
    nlp_requirement_met = (not settings.nlp_enabled) or app.state.nlp_ready
    ...
    logger.info(
        ...
        extra={
            ...
            "typo_enabled": bool(app.state.typo_engine is not None),
...
def create_app(
    ...
    async def lifespan(app: FastAPI):
        applied = _init_db(app, app_settings)
        _init_nlp(app, app_settings, nlp_adapter_factory)
        _init_cor_local(app, app_settings)
        _init_cor(app, app_settings)
        _init_typo(app, app_settings)
        _init_translation(app, app_settings)
        _init_word_verification(app, app_settings)
        _init_tts(app, app_settings)
        _log_startup(app, app_settings, applied)
        yield
        for _attr in ("cor_lexicon_service", "translation_service", "word_verification_service", "tts_service"):
            svc = getattr(app.state, _attr, None)
