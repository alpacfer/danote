...
class NumbersAudioRepository:
    ...
    def get(self, term: str) -> PronunciationAudio | None:
        with get_connection(self._db_path) as conn:
            row = conn.execute(
                "SELECT audio, mime_type FROM numbers_reference_audio WHERE term = ? LIMIT 1",
                (term.strip().lower(),),
            ).fetchone()
        if row is None:
            return None
    ...
    def upsert(
        ...
        with get_connection(self._db_path) as conn:
            conn.execute(
                """
                INSERT INTO numbers_reference_audio (term, audio, mime_type, provider, model, generated_at)
