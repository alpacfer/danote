...
Tool-agnostic AI agent context for danote. Claude, Codex, and other coding agents should treat this as the source of truth.
...
Use these roles as optional mental models or tool-specific subagents when available:

- `test-writer`: targeted backend/frontend tests after implementation or for coverage gaps.
- `docs-updater`: docs audit after API/schema/workflow/behavior changes.
- `danish-linguist`: Danish morphology, token classification, COR lexicon, translation, verification behavior.
- Explorer/research role: broad codebase navigation or external docs research.
- Planner role: implementation strategy for broad changes.
